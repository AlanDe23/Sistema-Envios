...
namespace Envios.Application.Services
{
    public class PedidoServiceDelivery
    {
        ...
        public async Task CambiarEstadoEnTransitoAsync(UpdatePedidoEnTransitoDto dto)
        {
            var pedido = await _pedidoRepo.GetByIdAsync(dto.IdPedido);
            if (pedido == null)
        ...
        public async Task CambiarEstadoEntregadoAsync(UpdatePedidoEntregadoDto dto)
        {
            var pedido = await _pedidoRepo.GetByIdAsync(dto.IdPedido);
            if (pedido == null)
                throw new Exception("Pedido no encontrado");
            ...
            await _balanceService.ActualizarBalanceDeliveryAsync(pedido.IdDelivery.Value);
        }

        public async Task CambiarEstadoNoEntregadoAsync(UpdatePedidoNoEntregadoDto dto)
        {
            var pedido = await _pedidoRepo.GetByIdAsync(dto.IdPedido);
            if (pedido == null)
                throw new Exception("Pedido no encontrado");
        ...
        /// </summary>
        public async Task<object> ObtenerResumenPedidosAsync(DateTime fecha)
        {
        ...
        public async Task<object> ObtenerPedidosPorDeliveryAsync(int idDelivery)
        {
            var pedidos = await _pedidoRepo.GetPedidosPorDeliveryAsync(idDelivery);
